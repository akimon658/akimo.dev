import {
  ...
  specifier,
  toSourceMap,
} from "lume/deps/tailwindcss.ts";
import { merge } from "lume/core/utils/object.ts";
import { log, warnUntil } from "lume/core/utils/log.ts";
import { dirname, toFileUrl } from "lume/deps/path.ts";
import { readFile } from "lume/core/utils/read.ts";
import { resolveInclude } from "lume/core/utils/path.ts";
import { prepareAsset, saveAsset } from "lume/plugins/source_maps.ts";
import { Features, transform } from "lume/deps/lightningcss.ts";
import { getFile, isFromCdn } from "lume/core/utils/cdn.ts";

import type { ChangedContent } from "lume/deps/tailwindcss.ts";
      ...
      for (const file of files) {
        ...
        const compiler = await compile(content, {
          ...
          async loadModule(id, base, resourceHint) {
            if (id.startsWith(".")) {
              id = site.src(base, id);
              const mod = await import(toFileUrl(id).href);
              return {
                base,
                path: id,
                module: mod.default,
